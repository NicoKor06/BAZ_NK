...
- Erstelle die API’s nach der vorgegebenen OpenAPI Spezifikation (openapi.json)
    ...
    - Ergänze die fehlenden Methoden in der Spezifikation (aktuell sind alle Routen auf "GET" gemapped, ist das korrekt?
      Schau auf die summary, was die Route machen soll.)
...
- Speichere im 1. Schritt (weil's vorerst einfacher ist) die Blog-Einträge z.B. in einer Runtime-Variable oder in einem
  JSON-File (und wenn du dich bereit fühlst, in einer Datenbank.. hier kommt Docker ins Spiel)
...
- Was ist eine gute Applikation ohne Unit-Tests? Informiere dich über Testing in Go und versuche eine möglich hohe
  Test-Abdeckung für deinen Code zu generieren
- Stelle Fragen 😊
- Deine Backend-Applikation ist fertig und du willst noch ein schönes Frontend dazu bauen? Dann findest du hier alle
  Infos:
